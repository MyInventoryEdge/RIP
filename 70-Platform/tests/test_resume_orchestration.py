...
class ResumeOrchestrationTests(unittest.TestCase):
    ...
    def test_ready_run_performs_fresh_verification_and_advances_only_through_lifecycle(self) -> None:
        result = resume_governed_onboarding(workspace=self.root, onboarding_run_id=self.context.onboarding_run_id)
    ...
    def test_changed_source_interrupts_and_preserves_immutable_history(self) -> None:
        ...
        self.assertEqual(ClassificationReadiness.STALE_SOURCE, result.readiness)
        self.assertIn("artifacts were preserved", result.message)
        self.assertEqual("interrupted", json.loads((self.run / "state.json").read_text(encoding="utf-8"))["state"])
        self.assertTrue((self.run / "resume-integrity-difference.json").is_file())
