...
class ClassificationReviewTests(unittest.TestCase):
 def test_read_only_review_reports_preserved_work_and_requests(self):
  with tempfile.TemporaryDirectory() as temp:
   parent=Path(temp); root=parent/'acme-org'; root.mkdir(); (root/'workspace.json').write_text('{"schema":"rip.organization-workspace.v1","organization_id":"acme-org"}',encoding='utf-8'); run=root/'onboarding-runs'/'run-001'; (run/'classifications'/'requests').mkdir(parents=True)
   ...
   review=load_classification_review(parent,'acme-org','run-001'); text=format_classification_review(review)
   self.assertIn('Onboarding paused safely',text); self.assertIn('state.json',text); self.assertIn('observation.json',text); self.assertEqual(review,load_classification_review(parent,'acme-org','run-001'))
